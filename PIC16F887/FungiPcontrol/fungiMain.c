...
int globalVsetpoint;
int globalVmeasured;
int controlSignalFan;
int controlSignalHeater;
...
void __interrupt() Timer0Interrupt(){
    ...
    globalVmeasured = ADRESH;
    
    
    // Update fan control:
    controlSignalFan = (+ globalVmeasured - globalVsetpoint );
    if( controlSignalFan < VfanMin ){
        controlSignalFan = VfanMin;
    }
    else{
        controlSignalFan = Kfan*controlSignalFan;
    }    
    
    controlSignalFan = controlSignalFan; 
    
    CCPR1L = controlSignalFan;
    
    // Update heater control:
    controlSignalHeater = KpHeater*(- globalVmeasured + globalVsetpoint);
    if(controlSignalHeater < 0){
        controlSignalHeater = 0;
    }
        
    CCPR2L = controlSignalHeater;
